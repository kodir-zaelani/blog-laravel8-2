                        ...
                        <div class="col-lg-12 col-md-12 col-12">
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <th class="text-right" scope="row">{{ $no + $users->firstItem() }}</th>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->email }}</td>
                                            <td> @if (!empty($item->photo))
                                                <img width="100px" class="img-fluid" src="{{ url('uploads/photos/users/photos_thumb/'. $item->photo) }}" />
                                                @else
                                                No featured image available!
                                                @endif</td>
                                                <td class="text-center">
                                                    {{-- <button wire:click="selectItem({{ $item->id }}, 'edit')" class="mb-2 btn btn-sm btn-warning" itle="Edit"><i class="fa fa-edit"></i></button> --}}
                                                    <a href="{{ route('admin.users.edit', $item->slug) }}" class="mt-2 btn btn-sm btn-warning"><i class="fa fa-pencil-alt"></i></a>
                                                    
                                                    @if($item->id == config('cms.default_user_id') || $item->id == $currentUser->id)
                                                    
                                                    @else
                                                    <button wire:click="selectItem({{ $item->id }}, 'delete')"  class="mt-2 btn btn-sm btn-danger" title="Delete"><i class="fas fa-trash-alt"></i>
                                                    </button>
                                                    @endif 
                            ...
                            <div class="mt-3 row">
                                ...
                                <div class="text-right col">
                                    Page : {{ $users->currentPage() }} | Show {{ $users->count() }} data of {{ $users->total() }}
                                </div>
                            </div>
                            {{-- Modal Add New--}}
                                ...
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="modalFormLabel">New User</h5>
                            ...
                            <div class="modal fade" id="modalEditForm" tabindex="-1" aria-labelledby="modalEditFormLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="modalEditFormLabel">Edit User</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
